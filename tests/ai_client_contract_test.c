...
#define CAP_SCREENKEY_STATE RAWHID_APP_CAP_AI_CLIENT_SCREENKEY_STATE
...
static void test_capability_bit_values(void) {
    ...
    assert(CAP_SCREENKEY_STATE == (1u << 14));
    assert(RAWHID_APP_AI_CLIENT_DISPLAY_SLOT_MAX == 7);
}

static void test_core_only_advertises_nothing(void) {
    assert(rawhid_app_ai_client_capability_bits(false, false, false, false, false) == 0);
    assert(rawhid_app_ai_client_capability_bits(true, false, false, false, false) == 0);
}

static void test_core_and_renderer_advertise_state_and_work_phase(void) {
    assert(rawhid_app_ai_client_capability_bits(true, true, false, false, false) ==
           (CAP_STATE | CAP_WORK_PHASE));
}

static void test_claude_code_renderer_adds_bit_12(void) {
    assert(rawhid_app_ai_client_capability_bits(true, true, true, false, false) ==
           (CAP_STATE | CAP_WORK_PHASE | CAP_CLAUDE_CODE));
}

static void test_claude_code_bit_is_never_advertised_alone(void) {
    assert(rawhid_app_ai_client_capability_bits(false, false, true, false, false) == 0);
    assert(rawhid_app_ai_client_capability_bits(true, false, true, false, false) == 0);
    assert(rawhid_app_ai_client_capability_bits(false, true, true, false, false) == 0);
}

static void test_display_slot_renderer_adds_bit_13(void) {
    assert(rawhid_app_ai_client_capability_bits(true, true, false, true, false) ==
           (CAP_STATE | CAP_WORK_PHASE | CAP_DISPLAY_SLOT));
    assert(rawhid_app_ai_client_capability_bits(true, true, true, true, false) ==
...
static void test_display_slot_bit_is_never_advertised_alone(void) {
    assert(rawhid_app_ai_client_capability_bits(false, false, false, true, false) == 0);
    assert(rawhid_app_ai_client_capability_bits(true, false, false, true, false) == 0);
    assert(rawhid_app_ai_client_capability_bits(false, true, false, true, false) == 0);
    assert(rawhid_app_ai_client_capability_bits(false, true, true, true, false) == 0);
}
...
static void test_legacy_device_without_slot_renderer_is_unchanged(void) {
    ...
    assert((rawhid_app_ai_client_capability_bits(true, true, true, false, false) & CAP_DISPLAY_SLOT) == 0);
    assert((rawhid_app_ai_client_capability_bits(true, true, false, false, false) & CAP_DISPLAY_SLOT) == 0);
}

static void test_screenkey_state_renderer_adds_bit_14(void) {
    assert(rawhid_app_ai_client_capability_bits(true, true, true, true, true) ==
           (CAP_STATE | CAP_WORK_PHASE | CAP_CLAUDE_CODE | CAP_DISPLAY_SLOT | CAP_SCREENKEY_STATE));
    assert(rawhid_app_ai_client_capability_bits(false, true, true, true, true) == 0);
    assert((rawhid_app_ai_client_capability_bits(true, true, true, false, true) &
            CAP_SCREENKEY_STATE) == 0);
}

static void test_screenkey_state_bit_always_comes_with_slots(void) {
    for (unsigned int combination = 0; combination < 32u; combination++) {
        const uint32_t caps = rawhid_app_ai_client_capability_bits(
            (combination & 1u) != 0, (combination & 2u) != 0, (combination & 4u) != 0,
            (combination & 8u) != 0, (combination & 16u) != 0);
        if ((caps & CAP_SCREENKEY_STATE) != 0) {
            assert((caps & CAP_STATE) != 0);
            assert((caps & CAP_WORK_PHASE) != 0);
            assert((caps & CAP_DISPLAY_SLOT) != 0);
        }
    }
}
...
int main(void) {
    ...
    test_screenkey_state_renderer_adds_bit_14();
    test_screenkey_state_bit_always_comes_with_slots();
