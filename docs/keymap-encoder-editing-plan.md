...
- `&mmv` / `&msc` (`zmk,behavior-input-two-axis`) は press 中の経過時間に基づいて relative input を生成する。runtime override は標準のsensor rotateと同じbehavior queueへ、各triggerの`press(tap-ms)`と`release(0)`を追加する。`tap-ms`は`.keymap`で指定した`keylink,encoder-runtime.sensor-behavior`の値を使うため、直接の`input_report_rel()`や固定detent変換を持たない。
